...
const frontDoorClient = createClient(
  import.meta.env.VITE_FRONT_DOOR_SUPABASE_URL as string,
  import.meta.env.VITE_FRONT_DOOR_SUPABASE_ANON_KEY as string,
);
...
export async function syncToFrontDoor(payload: ElmSyncPayload): Promise<void> {
  try {
    const { data: { user } } = await frontDoorClient.auth.getUser();
...
async function upsertElmState(userId: string | null, activeProjectCount: number, priorityProject: string | null, nextDeadlineTitle: string | null, nextDeadlineDate: string | null, pendingReviewCount: number) {
  ...
  const query = userId
    ? frontDoorClient.from('elm_state').select('id').eq('user_id', userId).order('updated_at', { ascending: false }).limit(1).maybeSingle()
    : frontDoorClient.from('elm_state').select('id').is('user_id', null).order('updated_at', { ascending: false }).limit(1).maybeSingle();
  const { data: existing } = await query;
  if (existing) { await frontDoorClient.from('elm_state').update(payload).eq('id', existing.id); }
  else { await frontDoorClient.from('elm_state').insert({ ...payload, user_id: userId }); }
}
...
async function upsertDailyItems(topProjects: ElmProject[], priorityTaskTitle: string | null, userId: string | null) {
  ...
  await Promise.all(items.map(async (item, idx) => {
    ...
    const { data: existing } = await frontDoorClient.from('daily_items').select('id, completion_state').eq('source_id', sourceId).maybeSingle();
    if (existing) return;
    await frontDoorClient.from('daily_items').insert({ source_app: 'elm', source_id: sourceId, title: item.title, domain: 'work', priority: idx + 1, energy_fit: 'high', estimated_minutes: 60, due_today: true, scheduled_date: today, completion_state: 'pending', is_hero: idx === 0, display_order: 20 + idx, user_id: userId });
  }));
}

async function upsertContextSnapshot(upcomingDeadlines: Array<{ title: string; date: string }>) {
  await frontDoorClient.from('context_snapshots').insert({ energy_level: 'medium', weekly_focus: 'Project delivery', capacity: 'full', stress_level: 'normal', upcoming_deadlines: upcomingDeadlines });
}
