...
type BoardMode = "project" | "library";
...
export default function SpatialCanvas({ projectId, projectName: _projectName, onBackToProject: _onBackToProject, initialDrawer = null }: SpatialCanvasProps) {
  ...
  const refreshCanvasFromServer = useCallback(async () => {
    if (!selectedBoardId) return;
    const els = await loadCanvasElements(selectedBoardId);
    setElements(els);
  ...
  useEffect(() => {
    ...
    loadCanvasElements(selectedBoardId)
      .then(async (data: CanvasElement[]) => {
        ...
        setElements(data);
        if (data.length > 0) {
          setMaxZ(Math.max(...data.map((e) => e.z_index)) + 1);
        }
        setLoading(false);
      ...
      .catch((err) => {
        console.error("[Board] Failed to load canvas elements:", err);
        toast({ title: "Failed to load board elements", description: String(err?.message ?? err), variant: "destructive" });
        setLoading(false);
      });
  }, [selectedBoardId]);
    ...
    const hitTestForConnector = (clientX: number, clientY: number): CanvasElement | null => {
      ...
      Object.values(elements).forEach((el) => {
        ...
        if (wx >= el.x && wx <= el.x + w && wy >= el.y && wy <= el.y + h) {
          const z = el.z_index || 0;          if (z > bestZ) { bestZ = z; best = el; }
        }
      });
      return best;
    };
    ...
    try {
      ...
      if (targetElementId) {
        ...
        const el = await createCanvasElement(selectedBoardId, {
          ...
          height: 200,
          z_index: newZ,
          content: { url: result.url, caption: "" },
        });
        addElement(el);
        sendElementAdd(el);
      }
  ...
  const handleDuplicateElement = async (id: number) => {
    ...
    setMaxZ((z: number) => z + 1);
    try {
      const newEl = await createCanvasElement(selectedBoardId, {
        type: el.type,
        x: el.x + 30,
        y: el.y + 30,
        width: el.width,
  ...
  const handleElementTouchStart = (elementId: number) => (e: React.TouchEvent) => {
    ...
    longPressArmRef.current = {
      ...
      timer: setTimeout(() => {
        ...
        const newZ = maxZ;
        setMaxZ((z) => z + 1);
        updateElement(elementId, { z_index: newZ });
      }, LONG_PRESS_MS),
    };
  };
  ...
  const saveAnnotationStroke = useCallback(async (elementId: number, stroke: Stroke) => {
    ...
    try {
      await updateCanvasElement(elementId, { content: nextContent });
    } catch {
              ...
              {(actualRole === "admin" || actualRole === "crew") && (
                <>
                  ...
                  {selectedBoardId !== null && (
                    <DropdownMenuItem
                      onClick={() => {
                        const el = document.querySelector<HTMLElement>('[data-testid="button-versions"]');
                        el?.click();
                      }}
      ...
      {boards.length > 0 && selectedBoardId && boardMode === "library" && libraryView === "covers" && (
        <LibraryCollectionsView
          ...
          onReorderItems={(orderedIds) => {
            // Persist by z_index on each affected element; ascending starting at 1.
              ...
              if (libraryPayloadRaw && selectedBoardId) {
                try {
                  ...
                  if (toolType === "surface-paint" && payload && payload.kind === "paint") {
                    ...
                    createCanvasElement(selectedBoardId, {
                      type: "surface",
                      x: canvasX,
                      y: canvasY,
                      width: def.width,
              ...
              {elementsList.map((el) => {
                ...
                return (
                  <div
                    ...
                    style={{ left: el.x, top: el.y, width: w, height: h, overflow: "hidden", zIndex: (el.z_index || 0) + 0.5 }}
